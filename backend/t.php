...
// $result = checkThirtyDays($inputDate);
// if ($result) {
//     return json_encode([]);
// }
...
if(!empty($data["data"])){
    foreach($data["data"] as $fixture){
        ...
        // --- Correctly assign home & away using meta.location ---
        $homeTeam = null;
        $awayTeam = null;
        foreach ($fixture["participants"] as $team) {
            if (($team["meta"]["location"] ?? "") === "home") {
                $homeTeam = $team;
            } elseif (($team["meta"]["location"] ?? "") === "away") {
                $awayTeam = $team;
            }
        }
        ...
        $homeLogo = $homeTeam["image_path"] ?? "./assets/images.jpg";
        $awayLogo = $awayTeam["image_path"] ?? "./assets/images.jpg";

        // Map type_id to market text in predictions
        $predictions = mapPredictions($fixture['predictions'] ?? [], $marketTypes);

        // Placeholder stats (replace with real stats if available)
...
function checkThirtyDays($inputDate)
{
    $file = __DIR__ . '/start_date.json';

    // 1️⃣ Create or read stored date
    if (!file_exists($file)) {
        file_put_contents($file, json_encode(['start_date' => $inputDate]));
    }

    $data = json_decode(file_get_contents($file), true);
    $storedDate = new DateTime($data['start_date']);
    $now = new DateTime();

    // 2️⃣ Calculate difference
    $interval = $storedDate->diff($now)->days;

    // 3️⃣ If > 30 days, return empty JSON
    if ($interval > 30) {
        return true;
    } else {
        return false;

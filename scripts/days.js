...
function renderMatches(matches) {
  // --- Inject custom CSS for scrollbar + slides once ---
  ...
  matches.forEach((match, index) => {
    ...
    card.innerHTML = `
      <div style="color: #fff; margin-bottom: .9rem; font-weight: 700; font-size: 1.05rem;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: .9rem;">
        <div style="text-align: center;">
          <img src="${match.homeLogo || 'placeholder.png'}" style="width: 50px; height: 50px; object-fit: contain; margin-bottom: 6px;" />
          <div style="font-weight: 700; color: #ffd700;">${match.home}</div>
        </div>
        <div style="font-size: 1.15rem; font-weight: bold; color: #fff;">${match.score || "vs"}</div>
        <div style="text-align: center;">
          <img src="${match.awayLogo || 'placeholder.png'}" style="width: 50px; height: 50px; object-fit: contain; margin-bottom: 6px;" />
      ...
      <div style="margin-bottom: .9rem;">
        ...
        <div style="height: 20px; display: flex; border-radius: 8px; overflow: hidden; margin-top: 6px;">
          <div style="width: ${homeWidth}%; background-color: #dc3545; color: white; text-align: center; line-height: 20px;">
            ${match.home} 
          </div>
          <div style="width: ${drawWidth}%; background-color: #6c757d; color: white; text-align: center; line-height: 20px;">
            Draw
          </div>
          <div style="width: ${awayWidth}%; background-color: #0dcaf0; color: white; text-align: center; line-height: 20px;">
            ${match.away}
          </div>
        </div>
      </div>` : ""}
      ...
      <!-- Explore Details -->
      <div id="${exploreWrapperId}" style="text-align: center; padding: 12px 0;">
      ...
      <div id="${detailsId}" style="display: none; margin-top: .8rem; background: #1f2e4a; color: #ffd700; font-weight: 700; border-radius: 10px; padding: 10px;">
        <h5 style="margin: 0;">OVERALL</h5>
        <hr />
        <div style="text-transform: uppercase;"><strong>🔮 Prediction:</strong> ${mainPrediction}</div>
        <div style="margin-top: .8rem; color: #ffd700;"><strong>📈 Expected Goals (xG):</strong> ${xgHome} - ${xgAway}</div>
        <div style="margin-top: .8rem;"><strong>⚡ Pressure Index:</strong> 
          <div style="height: 18px; background: #444; border-radius: 10px; overflow: hidden; margin-top: 4px;">
            <div style="width: ${pressureIndex}%; background: #ffc107; height: 100%; text-align: center; line-height: 18px; color: #000;">${pressureIndex}%</div>
          </div>
        </div>
        <div style="margin-top: .8rem;"><strong>🎯 Confidence:</strong> ${mainConfidence}%</div>
        <div style="margin-top: .8rem;"><strong>✅ Value Bet:</strong> <span style="color:${valueBet ? '#0f0' : '#dc3545'};">${valueBet ? "Yes" : "No"}</span></div>
        <div style="margin-top: .8rem;"><strong>💰 Odds:</strong> Home ${odds.home} • Draw ${odds.draw} • Away ${odds.away}</div>
        <div style="margin-top: .8rem;"><strong>📺 Live Odds:</strong> ${liveOdds || "Not available"}</div>

        <!-- Slider -->
        <div class="slider-container" style="overflow-x: auto; display: flex; gap: 12px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; margin-top: .8rem;">
          ${match.predictions && match.predictions.length ? match.predictions.map(pred => {
            ...
            if (pred.market === "Clean Sheet" && pred.values && pred.values.scores) {
              const rows = Object.entries(pred.values.scores)
                ...
                `).join("");
              return `
                <div class="slide" style="flex: 0 0 85%; min-width: 220px; max-height:240px; overflow-y:auto; font-size:0.8rem;">
              ...
              return `
                <div class="slide" style="flex: 0 0 65%; min-width: 160px; font-size:0.85rem;">
                  <div><strong>🔮 Market:</strong> ${pred.market}</div>
